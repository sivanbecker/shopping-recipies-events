...
import type { EventMemberWithProfile } from '@/types'
...
export function ShareEventDialog({ eventId, onClose }: Props) {
  ...
  const { data: members = [], isLoading } = useQuery<EventMemberWithProfile[]>({
    queryKey: ['event_members', eventId],
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_event_members', { p_event_id: eventId })
  ...
  const addMemberMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['all_event_members'] })
      setEmail('')
      setEmailError(null)
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
        ...
        <div className="mb-5">
          ...
          {isLoading ? (
            ...
            <ul className="space-y-2">
              {members.map(member => (
                <li key={`${member.user_id}-${member.role}`} className="flex items-center gap-2">
                  <UserAvatar
                  ...
                  {member.role === 'owner' ? (
                    <span className="rounded-lg bg-purple-100 px-2 py-1 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">
                      {t('sharing.roleOwner')}
                    </span>
                  ) : (
                    <>
                      <select
                        ...
                        className="rounded-lg border border-gray-200 bg-gray-50 px-2 py-1 text-xs text-gray-700 focus:border-purple-400 focus:outline-none focus:ring-1 focus:ring-purple-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200"
                      >
        ...
        <div>
          ...
          <input
            ...
            className="mb-2 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500"
            autoComplete="email"
          />
          ...
          <div className="flex gap-2">
            <select
              ...
              className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700 focus:border-purple-400 focus:outline-none focus:ring-2 focus:ring-purple-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200"
            >
              <option value="editor">{t('sharing.roleEditor')}</option>
              <option value="viewer">{t('sharing.roleViewer')}</option>
            </select>
            <button
              ...
              className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-purple-500 px-4 py-2 text-sm font-semibold text-white hover:bg-purple-600 disabled:opacity-50"
            >
